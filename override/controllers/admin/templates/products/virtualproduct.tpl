	...
	<div id="virtual_good" {if !$product->productDownload->id || $product->productDownload->active}style="display:none"{/if} class="form-group">
		...
		<div id="is_virtual_file_product" style="display:none;">
			...
			{if empty($product->cache_default_attribute)}
				...
				<div class="form-group">
					<label class="control-label col-lg-3 required">{l s='Custom download link' mod='customdownloadlink'}</label>
					<div class="col-lg-5">
						<input type="text" id="virtual_product_custom_link" name="virtual_product_custom_link" value="{$product->productDownload->custom_link|escape:'html':'UTF-8'}" />
						<p class="help-block">{l s='A custom URL to the contents (e.g. http://example.com/files/Book.pdf) that will take precedence over any uploaded file.' mod='customdownloadlink'}</p>
					</div>
				</div>
				{if $is_file}
				<div class="form-group">
